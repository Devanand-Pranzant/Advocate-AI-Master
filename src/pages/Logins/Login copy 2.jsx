...
import { scheduleSessionExpiry } from "../../utils/sessionManager";
...
const LoginPage = ({ setUser, isAuthenticated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // scheduleSessionExpiry(res.data.token, handleSessionExpired);
      // scheduleSessionExpiry(res.data.token);
      scheduleSessionExpiry(res.data.token, () => {
        // alert("Session expired. Please login again.");
        localStorage.clear();
        setUser(null);
      ...
      // Reload once so App.jsx timers start
      // window.location.reload();

      // ✅ Update App state so routes & components react immediately
      // setUser(userData);
      // setIsAuthenticated(true);
